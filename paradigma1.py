...
def read_futoshiki(filename):
    with open(filename) as f:
        lines = [line.strip() for line in f if line.strip()]
    grid = [list(map(int, lines[i].split())) for i in range(4)]
    h_constraints = [lines[4+i].split() for i in range(4)]  # 4 linhas, 3 símbolos cada
    v_constraints = [lines[8+i].split() for i in range(3)]  # 3 linhas, 4 símbolos cada
...
def is_valid(grid, row, col, num, h_constraints, v_constraints):
    ...
    for i in range(4):
        if grid[row][i] == num or grid[i][col] == num:
            return False
    # Checa restrições horizontais (entre colunas)
    if col < 3:
        op = h_constraints[row][col]
        if op == '<' and grid[row][col+1] != 0 and not (num < grid[row][col+1]):
            return False
        if op == '>' and grid[row][col+1] != 0 and not (num > grid[row][col+1]):
            return False
    if col > 0:
        op = h_constraints[row][col-1]
        if op == '<' and grid[row][col-1] != 0 and not (grid[row][col-1] < num):
            return False
        if op == '>' and grid[row][col-1] != 0 and not (grid[row][col-1] > num):
            return False
    # Checa restrições verticais (entre linhas)
    if row < 3:
        op = v_constraints[row][col]
        if op == 'v' and grid[row+1][col] != 0 and not (num < grid[row+1][col]):
            return False
        if op == '^' and grid[row+1][col] != 0 and not (num > grid[row+1][col]):
            return False
    if row > 0:
        op = v_constraints[row-1][col]
        if op == 'v' and grid[row-1][col] != 0 and not (grid[row-1][col] < num):
            return False
        if op == '^' and grid[row-1][col] != 0 and not (grid[row-1][col] > num):
            return False
    return True

def solve(grid, h_constraints, v_constraints, pos=0):
    if pos == 16:
        return True
    row, col = divmod(pos, 4)
    if grid[row][col] != 0:
        return solve(grid, h_constraints, v_constraints, pos+1)
    for num in range(1, 5):
        if is_valid(grid, row, col, num, h_constraints, v_constraints):
            grid[row][col] = num
            if solve(grid, h_constraints, v_constraints, pos+1):
                return True
            grid[row][col] = 0
    return False
...
if __name__ == "__main__":
    grid, h_constraints, v_constraints = read_futoshiki("puzzle.txt")
